...
start_services() {
    ...
    ssh -i ~/.ssh/id_rsa "$SERVER_USER@$SERVER_HOST" << 'EOF'
        ...
        if docker ps | grep -q crafty-controller; then
            echo "✅ Crafty Controller is running!"
        else
            echo "❌ Crafty Controller failed to start"
            docker-compose logs
            exit 1
        fi
EOF
...
verify_deployment() {
    ...
    ssh -i ~/.ssh/id_rsa "$SERVER_USER@$SERVER_HOST" << 'EOF'
        ...
        if docker ps | grep -q crafty-controller; then
            ...
            echo "❌ Crafty Controller is not running"
            exit 1
        fi
        
        # Check if web panel port is accessible
        if nc -z localhost 8000; then
            echo "✅ Web panel port 8000 is accessible"
        else
            echo "❌ Web panel port 8000 is not accessible"
            exit 1
        fi
